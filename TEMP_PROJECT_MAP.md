...
**Role:** The module `agent_graph_main.py` Orchestrates searching for files ending in '_main.py' within a specified folder, initializes and runs the CrawlerAgent with the goal and target root, and returns a completion message indicating successful execution. [1]

### 🚨 Alerts
- TODO **TODO**: Implement the rest of the function `(Ref: Comment)`

### 🧩 Interface & Logic
- **`🔌 main`**: The function searches the specified folder for files ending in `_main.py`, initializes `CrawlerAgent` with the goal and target root, runs the agent, and returns a completion message indicating successful execution. [2]

### 🔗 Uses (Upstream)
- **`agent_core.py`**: Uses `agent_core.py`: Instantiates CrawlerAgent for crawling.. [3]

<details><summary><i>View Verification Claims</i></summary>

> 🆔 `39475b` [1]: Orchestrates searching for files ending in '_main.py' within a specified folder, initializes and runs the CrawlerAgent with the goal and target root, and returns a completion message indicating successful execution. _(Source: Synthesis (based on [2]))_
> 🆔 `6df37f` [2]: The function searches the specified folder for files ending in `_main.py`, initializes `CrawlerAgent` with the goal and target root, runs the agent, and returns a completion message indicating successful execution. _(Source: main)_
> 🆔 `977be4` [3]: Uses `agent_core.py`: Instantiates CrawlerAgent for crawling.. _(Source: Import agent_core.py)_
</details>
...
**Role:** The module `agent_core.py` The Agent Core coordinates the retrieval of project metadata, analysis of dependencies and APIs, generation of summaries using synthesizers, and creation of documentation by renderers.
...
### 🧩 Interface & Logic
- **`🔌 class CrawlerAgent`**: The CrawlerAgent class is responsible for initializing an agent with a goal and target root, creating memory storage using ChromaMemory, retrieving project map and processing order via `project_pulse`, setting up gatekeeper and synthesizer components, generating system summaries and reports, and managing memory cleanup. [2]
- **`🔌 🔌 CrawlerAgent.run`**: The method runs the CrawlerAgent, retrieves project map and processing order using `project_pulse`, initializes gatekeeper and synthesizer, synthesizes system summary, renders report, cleans up memory for 5 turns, and returns a completion response. [3]

### 🔗 Uses (Upstream)
- **`memory_core.py`**: Uses `memory_core.py`: Instantiates ChromaMemory class for memory management.. [4]
- **`summary_models.py`**: Uses `summary_models.py`: Instantiates ModuleContext class.. [5]
- **`agent_util.py`**: Uses `agent_util.py`: Calls project_pulse on target graph. [6]
- **`report_renderer.py`**: Uses `report_renderer.py`: Instantiates ReportRenderer class with project map and system summary.. [7]
- **`map_synthesizer.py`**: Uses `map_synthesizer.py`: Instantiates MapSynthesizer class.. [8]
- **`semantic_gatekeeper.py`**: Uses `semantic_gatekeeper.py`: Instantiates SemanticGatekeeper class.. [9]
- **`llm_util.py`**: Uses `llm_util.py`: Instantiates class from llm_util.. [10]
- **`agent_config.py`**: Uses `agent_config.py`: Instantiates class with DEFAULT_MODEL and CONTEXT_LIMIT. [11]
...
> 🆔 `dcb5f8` [1]: The Agent Core coordinates the retrieval of project metadata, analysis of dependencies and APIs, generation of summaries using synthesizers, and creation of documentation by renderers. _(Source: Synthesis (based on [2], [3]))_
> 🆔 `5838a6` [2]: The CrawlerAgent class is responsible for initializing an agent with a goal and target root, creating memory storage using ChromaMemory, retrieving project map and processing order via `project_pulse`, setting up gatekeeper and synthesizer components, generating system summaries and reports, and managing memory cleanup. _(Source: class CrawlerAgent)_
> 🆔 `ea8149` [3]: The method runs the CrawlerAgent, retrieves project map and processing order using `project_pulse`, initializes gatekeeper and synthesizer, synthesizes system summary, renders report, cleans up memory for 5 turns, and returns a completion response. _(Source: 🔌 CrawlerAgent.run)_
> 🆔 `257007` [4]: Uses `memory_core.py`: Instantiates ChromaMemory class for memory management.. _(Source: Import memory_core.py)_
> 🆔 `6f2803` [5]: Uses `summary_models.py`: Instantiates ModuleContext class.. _(Source: Import summary_models.py)_
> 🆔 `78786f` [6]: Uses `agent_util.py`: Calls project_pulse on target graph. _(Source: Import agent_util.py)_
> 🆔 `0314e6` [7]: Uses `report_renderer.py`: Instantiates ReportRenderer class with project map and system summary.. _(Source: Import report_renderer.py)_
> 🆔 `1808fc` [8]: Uses `map_synthesizer.py`: Instantiates MapSynthesizer class.. _(Source: Import map_synthesizer.py)_
> 🆔 `592dc2` [9]: Uses `semantic_gatekeeper.py`: Instantiates SemanticGatekeeper class.. _(Source: Import semantic_gatekeeper.py)_
> 🆔 `89463f` [10]: Uses `llm_util.py`: Instantiates class from llm_util.. _(Source: Import llm_util.py)_
> 🆔 `d217e0` [11]: Uses `agent_config.py`: Instantiates class with DEFAULT_MODEL and CONTEXT_LIMIT. _(Source: Import agent_config.py)_
</details>

---
## 📦 Module: `agent_util.py`
**Role:** The module `agent_util.py` The Agent Util module Orchestrates the integration of external services and APIs within the AI agent framework, ensuring effective interaction and data exchange between components.

**Impact Analysis:** Changes to this module will affect: agent_core.py [1]

### 🧩 Interface & Logic
- **`🔌 ProjectGraph`**: The statement assigns a dictionary literal using Dict[str, Any] as the data structure. [2]
- **`🔌 class ProjectSummarizer`**: The ProjectSummarizer class is designed to analyze a project graph, compute topological order of modules based on dependencies and dependents, generate module contexts for each path in the processing order while handling cycles, and update contexts using critiques from MapCritic. [3]
- **`🔌 project_pulse`**: The function takes a target file path, checks if it exists, initializes a GraphAnalyzer with the path, analyzes the project graph, creates a ProjectSummarizer with the graph, generates module contexts and processing order, and returns them. [4]
- **`🔌 🔌 ProjectSummarizer.generate_contexts`**: The method iterates through cycles, generating module contexts for each path in the processing order. It compares new and old contexts, updating them if necessary. Critiques from the MapCritic are used to provide instructions for each path. The method returns the updated module contexts dictionary and processing order list. [5]
- **`🔒 _create_module_context`**: The function `_create_module_context` generates module context by contextualizing the provided path, graph, and dependency contexts using `ModuleContextualizer`. It sets the file path attribute if not already present. [6]

### 🔗 Uses (Upstream)
- **`module_contextualizer.py`**: Uses `module_contextualizer.py`: Instantiates ModuleContextualizer with path, graph, dep_contexts.. [7]
- **`report_renderer.py`**: Uses `report_renderer.py`: Instantiates ReportRenderer class.. [8]
- **`summary_models.py`**: Uses `summary_models.py`: Instantiates `ModuleContext` for each module path.. [9]
- **`semantic_gatekeeper.py`**: Uses `semantic_gatekeeper.py`: Instantiates SemanticGatekeeper class.. [10]
- **`graph_analyzer.py`**: Uses `graph_analyzer.py`: Instantiates GraphAnalyzer for static analysis.. [11]
- **`map_critic.py`**: Uses `map_critic.py`: Calls MapCritic(gatekeeper) function.. [12]
...
> 🆔 `3d08be` [1]: The Agent Util module Orchestrates the integration of external services and APIs within the AI agent framework, ensuring effective interaction and data exchange between components. _(Source: Synthesis (based on [2], [5], [3], [6], [4]))_
> 🆔 `0135cc` [2]: The statement assigns a dictionary literal using Dict[str, Any] as the data structure. _(Source: ProjectGraph)_
> 🆔 `84e498` [3]: The ProjectSummarizer class is designed to analyze a project graph, compute topological order of modules based on dependencies and dependents, generate module contexts for each path in the processing order while handling cycles, and update contexts using critiques from MapCritic. _(Source: class ProjectSummarizer)_
> 🆔 `ecc783` [4]: The function takes a target file path, checks if it exists, initializes a GraphAnalyzer with the path, analyzes the project graph, creates a ProjectSummarizer with the graph, generates module contexts and processing order, and returns them. _(Source: project_pulse)_
> 🆔 `0fb817` [5]: The method iterates through cycles, generating module contexts for each path in the processing order. It compares new and old contexts, updating them if necessary. Critiques from the MapCritic are used to provide instructions for each path. The method returns the updated module contexts dictionary and processing order list. _(Source: 🔌 ProjectSummarizer.generate_contexts)_
> 🆔 `8f8ca7` [6]: The function `_create_module_context` generates module context by contextualizing the provided path, graph, and dependency contexts using `ModuleContextualizer`. It sets the file path attribute if not already present. _(Source: _create_module_context)_
> 🆔 `2ebc19` [7]: Uses `module_contextualizer.py`: Instantiates ModuleContextualizer with path, graph, dep_contexts.. _(Source: Import module_contextualizer.py)_
> 🆔 `390950` [8]: Uses `report_renderer.py`: Instantiates ReportRenderer class.. _(Source: Import report_renderer.py)_
> 🆔 `9604d7` [9]: Uses `summary_models.py`: Instantiates `ModuleContext` for each module path.. _(Source: Import summary_models.py)_
> 🆔 `f9ff16` [10]: Uses `semantic_gatekeeper.py`: Instantiates SemanticGatekeeper class.. _(Source: Import semantic_gatekeeper.py)_
> 🆔 `691a62` [11]: Uses `graph_analyzer.py`: Instantiates GraphAnalyzer for static analysis.. _(Source: Import graph_analyzer.py)_
> 🆔 `0d13d1` [12]: Uses `map_critic.py`: Calls MapCritic(gatekeeper) function.. _(Source: Import map_critic.py)_
</details>
