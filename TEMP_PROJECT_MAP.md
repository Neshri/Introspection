...
**Role:** The module `agent_graph_main.py` Orchestrates searching for main.py file in specified folder, initializing CrawlerAgent with goal and target root path, running the agent to process the project, synthesizing system summary, rendering report, cleaning up memory, and returning completion message. [1]

### 🚨 Alerts
- TODO **TODO**: Implement the rest of the function `(Ref: Comment)`

### 🧩 Interface & Logic
- **`🔌 main`**: Searches for `main.py` file in the specified folder, initializes CrawlerAgent with the goal and target root path, runs the agent to process the project, returns completion message [2]

### 🔗 Uses (Upstream)
- **`agent_core.py`**: Instantiates CrawlerAgent with goal and target_root. [3]

<details><summary><i>View Verification Claims</i></summary>

> 🆔 `9abe51` [1]: Orchestrates searching for main.py file in specified folder, initializing CrawlerAgent with goal and target root path, running the agent to process the project, synthesizing system summary, rendering report, cleaning up memory, and returning completion message. _(Source: Synthesis (based on [2]))_
> 🆔 `76d0d2` [2]: Searches for `main.py` file in the specified folder, initializes CrawlerAgent with the goal and target root path, runs the agent to process the project, returns completion message _(Source: main)_
> 🆔 `6cc9c2` [3]: Instantiates CrawlerAgent with goal and target_root. _(Source: Import agent_core.py)_
</details>
...
**Role:** The module `agent_core.py` `Agent Core` orchestrates the initialization and execution of a CrawlerAgent instance, which analyzes project structure to generate a system summary, renders a report, cleans up memory, and returns an analysis completion response.
...
### 🧩 Interface & Logic
- **`🔌 class CrawlerAgent`**: Initializes attributes such as goal, target root, and memory storage; runs the crawler to retrieve project map and processing order; synthesizes system summary and renders report; cleans up memory for 5 turns upon completion. [2]
- **`🔌 🔌 CrawlerAgent.run`**: Runs the CrawlerAgent, retrieves project map and processing order, synthesizes system summary, renders report, cleans up memory for 5 turns, and returns analysis completion response. [3]

### 🔗 Uses (Upstream)
- **`summary_models.py`**: Instantiates ModuleContext class. [4]
- **`memory_core.py`**: Instantiates class `ChromaMemory`. [5]
- **`llm_util.py`**: Imports class from `llm_util`. [6]
- **`agent_config.py`**: Imports `DEFAULT_MODEL`, `CONTEXT_LIMIT. [7]
- **`report_renderer.py`**: Instantiates ReportRenderer class. [8]
- **`agent_util.py`**: Calls project_pulse on target root. [9]
...
> 🆔 `fbbf33` [1]: `Agent Core` orchestrates the initialization and execution of a CrawlerAgent instance, which analyzes project structure to generate a system summary, renders a report, cleans up memory, and returns an analysis completion response. _(Source: Synthesis (based on [2], [3]))_
> 🆔 `390ea8` [2]: Initializes attributes such as goal, target root, and memory storage; runs the crawler to retrieve project map and processing order; synthesizes system summary and renders report; cleans up memory for 5 turns upon completion. _(Source: class CrawlerAgent)_
> 🆔 `f003b7` [3]: Runs the CrawlerAgent, retrieves project map and processing order, synthesizes system summary, renders report, cleans up memory for 5 turns, and returns analysis completion response. _(Source: 🔌 CrawlerAgent.run)_
> 🆔 `ca5374` [4]: Instantiates ModuleContext class. _(Source: Import summary_models.py)_
> 🆔 `25f950` [5]: Instantiates class `ChromaMemory`. _(Source: Import memory_core.py)_
> 🆔 `02c300` [6]: Imports class from `llm_util`. _(Source: Import llm_util.py)_
> 🆔 `f9a4a7` [7]: Imports `DEFAULT_MODEL`, `CONTEXT_LIMIT. _(Source: Import agent_config.py)_
> 🆔 `7b9009` [8]: Instantiates ReportRenderer class. _(Source: Import report_renderer.py)_
> 🆔 `4340a5` [9]: Calls project_pulse on target root. _(Source: Import agent_util.py)_
...
**Role:** The module `agent_util.py` Executes `execute_with_feedback` to generate responses based on provided inputs and returns specified keys from the output if expected.
...
- **`🔌 class ProjectSummarizer`**: Initializes attributes including project graph, max cycles, contexts dictionary, and processing order. [3]
- **`🔌 project_pulse`**: Analyzes project structure and roles by building dependency graph, summarizing modules and their contexts, returning final module contexts and processing order. [4]
- **`🔌 🔌 ProjectSummarizer.generate_contexts`**: Iterates over cycles to generate module contexts, updating based on dependencies and critiques, caching unchanged paths. [5]
- **`🔒 _create_module_context`**: Creates and returns module context by initializing ModuleContextualizer with path, graph, dependency contexts, and optional critique instruction, setting file_path if not present. [6]

### 🔗 Uses (Upstream)
- **`graph_analyzer.py`**: Instantiates GraphAnalyzer. [7]
- **`summary_models.py`**: Instantiates ModuleContext class. [8]
- **`map_critic.py`**: Instantiates MapCritic class with gatekeeper parameter. [9]
...
> 🆔 `5ad67b` [1]: Executes `execute_with_feedback` to generate responses based on provided inputs and returns specified keys from the output if expected. _(Source: Synthesis (based on [6], [2], [3], [5], [4]))_
> 🆔 `4ba765` [2]: Assigns a dictionary to the variable `ProjectGraph`. _(Source: ProjectGraph)_
> 🆔 `773bff` [3]: Initializes attributes including project graph, max cycles, contexts dictionary, and processing order. _(Source: class ProjectSummarizer)_
> 🆔 `e66f65` [4]: Analyzes project structure and roles by building dependency graph, summarizing modules and their contexts, returning final module contexts and processing order. _(Source: project_pulse)_
> 🆔 `9c5dcf` [5]: Iterates over cycles to generate module contexts, updating based on dependencies and critiques, caching unchanged paths. _(Source: 🔌 ProjectSummarizer.generate_contexts)_
> 🆔 `1e7415` [6]: Creates and returns module context by initializing ModuleContextualizer with path, graph, dependency contexts, and optional critique instruction, setting file_path if not present. _(Source: _create_module_context)_
> 🆔 `24021d` [7]: Instantiates GraphAnalyzer. _(Source: Import graph_analyzer.py)_
> 🆔 `45488d` [8]: Instantiates ModuleContext class. _(Source: Import summary_models.py)_
> 🆔 `b89a99` [9]: Instantiates MapCritic class with gatekeeper parameter. _(Source: Import map_critic.py)_
