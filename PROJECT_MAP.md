...
The system is an AI-driven agent designed to process and analyze project root directories, synthesizing system summaries through grounded synthesis and rendering structured reports while managing memory and dependencies. The entry point module, `agent_graph_main.py`, orchestrates the initialization of the agent by calling `agent_core.py`. This core module interacts with various services such as `semantic_gatekeeper.py` for output validation, `dependency_analyst.py` and `component_analyst.py` for analyzing dependencies and source code respectively, and `agent_util.py` for coordinating module contexts. The system also utilizes `module_contextualizer.py`, `map_synthesizer.py`, and `report_renderer.py` to render structured reports based on the analyzed data. Memory management is handled by `memory_core.py`. The agent's functionality is defined in `main.py`, which interacts with these modules to achieve its goals.
...
**Role:** The module `agent_graph_main.py` Orchestrates the initialization of an agent that processes a project root directory containing _main.py to define and run the agent's functionality. [1]

### 🚨 Alerts
- TODO **TODO**: Implement the rest of the function `(Ref: Comment)`

### 🧩 Interface & Logic
- **`🔌 main`**: Initializes agent, runs it to process the project root directory containing `_main.py`, and returns completion message [2]

### 🔗 Uses (Upstream)
- **`agent_core.py`**: Orchestrates AI-driven agent initialization, integrates project context retrieval, synthesizes system summaries via grounded synthesis, renders structured reports, manages memory and dependencies for five turns, and returns an analysis completion response. [3]

<details><summary><i>View Verification Claims</i></summary>

> 🆔 `38716e` [1]: Orchestrates the initialization of an agent that processes a project root directory containing _main.py to define and run the agent's functionality. _(Source: Synthesis (based on [2]))_
> 🆔 `1bd376` [2]: Initializes agent, runs it to process the project root directory containing `_main.py`, and returns completion message _(Source: main)_
> 🆔 `edb550` [3]: Orchestrates AI-driven agent initialization, integrates project context retrieval, synthesizes system summaries via grounded synthesis, renders structured reports, manages memory and dependencies for five turns, and returns an analysis completion response. _(Source: Import agent_core.py)_
</details>
...
**Role:** The module `agent_core.py` Orchestrates an AI-driven agent that initializes project context, synthesizes system summaries through grounded synthesis, and renders structured reports while managing memory and dependencies.
...
### 🧩 Interface & Logic
- **`🔌 class CrawlerAgent`**: Initializes an instance of CrawlerAgent by setting the goal and target root, creating a ChromaMemory memory instance, and printing initialization details. [2]
- **`🔌 🔌 CrawlerAgent.run`**: Initializes agent, retrieves project map and processing order, synthesizes system summary, renders report using renderer, cleans up memories for 5 turns, and returns analysis completion response. [3]

### 🔗 Uses (Upstream)
- **`agent_config.py`**: Configures agent behavior by setting global constants for model and context limit settings used throughout the agent's execution flow. [4]
- **`summary_models.py`**: Defines module's role, dependencies, dependents, public API, alerts, and claims to enable thorough analysis and documentation. [5]
- **`report_renderer.py`**: Orchestrates generation of project context map documentation by listing modules, categorizing them into archetype groups, writing formatted lines to an output file, and managing claims. [6]
- **`semantic_gatekeeper.py`**: Validates and extracts structured data from LLM responses while ensuring policy compliance. [7]
- **`memory_core.py`**: Manages and queries the collection of memories to optimize memory usage by updating embeddings, helpfulness scores, and cleaning up outdated or low-use memories. [8]
- **`agent_util.py`**: Orchestrates and coordinates module contexts, updating them iteratively based on critiques and dependencies. [9]
- **`map_synthesizer.py`**: Orchestrates system architecture synthesis by coordinating anchor specifics, supporting cast details, and project goal grounding through ranked paths based on relevance scores. [10]
- **`llm_util.py`**: Formats user input into messages for LLM model, prepares and sends queries to the chat function, retrieves response content while handling errors. [11]
...
> 🆔 `6e20ce` [1]: Orchestrates an AI-driven agent that initializes project context, synthesizes system summaries through grounded synthesis, and renders structured reports while managing memory and dependencies. _(Source: Synthesis (based on [2], [3]))_
> 🆔 `c3b5e4` [2]: Initializes an instance of CrawlerAgent by setting the goal and target root, creating a ChromaMemory memory instance, and printing initialization details. _(Source: class CrawlerAgent)_
> 🆔 `d45eb4` [3]: Initializes agent, retrieves project map and processing order, synthesizes system summary, renders report using renderer, cleans up memories for 5 turns, and returns analysis completion response. _(Source: 🔌 CrawlerAgent.run)_
> 🆔 `fee358` [4]: Configures agent behavior by setting global constants for model and context limit settings used throughout the agent's execution flow. _(Source: Import agent_config.py)_
> 🆔 `82ee2b` [5]: Defines module's role, dependencies, dependents, public API, alerts, and claims to enable thorough analysis and documentation. _(Source: Import summary_models.py)_
> 🆔 `5aabed` [6]: Orchestrates generation of project context map documentation by listing modules, categorizing them into archetype groups, writing formatted lines to an output file, and managing claims. _(Source: Import report_renderer.py)_
> 🆔 `a44831` [7]: Validates and extracts structured data from LLM responses while ensuring policy compliance. _(Source: Import semantic_gatekeeper.py)_
> 🆔 `8fa4c9` [8]: Manages and queries the collection of memories to optimize memory usage by updating embeddings, helpfulness scores, and cleaning up outdated or low-use memories. _(Source: Import memory_core.py)_
> 🆔 `681218` [9]: Orchestrates and coordinates module contexts, updating them iteratively based on critiques and dependencies. _(Source: Import agent_util.py)_
> 🆔 `24e8dc` [10]: Orchestrates system architecture synthesis by coordinating anchor specifics, supporting cast details, and project goal grounding through ranked paths based on relevance scores. _(Source: Import map_synthesizer.py)_
> 🆔 `e58802` [11]: Formats user input into messages for LLM model, prepares and sends queries to the chat function, retrieves response content while handling errors. _(Source: Import llm_util.py)_
</details>

---
## 📦 Module: `agent_util.py`
**Role:** The module `agent_util.py` Analyzes and coordinates module contexts, updating them iteratively based on critiques and dependencies.

**Impact Analysis:** Changes to this module will affect: agent_core.py [1]

### 🧩 Interface & Logic
- **`🔌 ProjectGraph`**: Assigns the data structure `Dict[str, Any]` to variable `ProjectGraph`. [2]
- **`🔌 class ProjectSummarizer`**: Initializes summarizer with project graph, maximum cycles for processing, an empty contexts dictionary, and computes the topological order. [3]
- **`🔌 project_pulse`**: Analyzes project graph, generates summarizer contexts, and returns final contexts and processing order. [4]
- **`🔌 🔌 ProjectSummarizer.generate_contexts`**: Iterates through cycles to generate module contexts, updating and caching them based on dependencies and critiques, returning the final context dictionary and processing order list. [5]
- **`🔒 _create_module_context`**: Creates or retrieves module context, sets file path, and returns ModuleContext. [6]

### 🔗 Uses (Upstream)
- **`graph_analyzer.py`**: Builds dependency graph to analyze Python module structure for static analysis. [7]
- **`summary_models.py`**: Aggregates and organizes metadata about module roles, dependencies, dependents, public API, alerts, and claims for use in downstream modules. [8]
...
- **`semantic_gatekeeper.py`**: Validates and parses LLM output from `agent_util.py`, ensuring structured data extraction and policy compliance. [10]
- **`module_contextualizer.py`**: Analyzes module capabilities and dependencies to generate contextualized documentation for downstream modules in agent_util.py, providing clear roles and relationships based on source code analysis. [11]
- **`map_critic.py`**: Analyzes project map content to generate critiques for up to three modules, defining documentation quality based on predefined criteria. [12]
...
> 🆔 `565996` [1]: Analyzes and coordinates module contexts, updating them iteratively based on critiques and dependencies. _(Source: Synthesis (based on [4], [3], [2], [6], [5]))_
> 🆔 `9e8ec2` [2]: Assigns the data structure `Dict[str, Any]` to variable `ProjectGraph`. _(Source: ProjectGraph)_
> 🆔 `72ba2c` [3]: Initializes summarizer with project graph, maximum cycles for processing, an empty contexts dictionary, and computes the topological order. _(Source: class ProjectSummarizer)_
> 🆔 `3b0d1e` [4]: Analyzes project graph, generates summarizer contexts, and returns final contexts and processing order. _(Source: project_pulse)_
> 🆔 `f0cf94` [5]: Iterates through cycles to generate module contexts, updating and caching them based on dependencies and critiques, returning the final context dictionary and processing order list. _(Source: 🔌 ProjectSummarizer.generate_contexts)_
> 🆔 `d3f5f6` [6]: Creates or retrieves module context, sets file path, and returns ModuleContext. _(Source: _create_module_context)_
> 🆔 `261b21` [7]: Builds dependency graph to analyze Python module structure for static analysis. _(Source: Import graph_analyzer.py)_
> 🆔 `8e77bc` [8]: Aggregates and organizes metadata about module roles, dependencies, dependents, public API, alerts, and claims for use in downstream modules. _(Source: Import summary_models.py)_
...
> 🆔 `186e30` [10]: Validates and parses LLM output from `agent_util.py`, ensuring structured data extraction and policy compliance. _(Source: Import semantic_gatekeeper.py)_
> 🆔 `3aa7b7` [11]: Analyzes module capabilities and dependencies to generate contextualized documentation for downstream modules in agent_util.py, providing clear roles and relationships based on source code analysis. _(Source: Import module_contextualizer.py)_
> 🆔 `8118a6` [12]: Analyzes project map content to generate critiques for up to three modules, defining documentation quality based on predefined criteria. _(Source: Import map_critic.py)_
</details>
