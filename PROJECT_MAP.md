...
The system's architecture is built on a solid foundation of configuration and data management layers. The Configuration layer defines constants and integrates core functionalities, influencing agent_core.py and semantic_gatekeeper.py. These configurations are crucial for setting up the system's parameters and behaviors. Supported by the Utility layer, which handles model processing, API requests to Ollama chat, response handling, and content formatting, these foundational elements ensure that data is correctly processed and manipulated throughout the application.
...
**Role:** The module `agent_graph_main.py` Orchestrates the execution of CrawlerAgent by initializing it with a goal and target root directory, running its run() method to process the agent's tasks, and returning a completion message. [1]

### 🚨 Alerts
- TODO **TODO**: Implement the rest of the function `(Ref: Comment)`

### 🧩 Interface & Logic
- **`🔌 main`**: Initializes CrawlerAgent, executes agent.run(), and returns completion message. [2]

### 🔗 Uses (Upstream)
- **`agent_core.py`**: Orchestrates project analysis by initializing CrawlerAgent, fetching and processing the project map, synthesizing system summaries, rendering reports, and cleaning up for five turns. [3]

<details><summary><i>View Verification Claims</i></summary>

> 🆔 `07d2d9` [1]: Orchestrates the execution of CrawlerAgent by initializing it with a goal and target root directory, running its run() method to process the agent's tasks, and returning a completion message. _(Source: Synthesis (based on [2]))_
> 🆔 `bf10e2` [2]: Initializes CrawlerAgent, executes agent.run(), and returns completion message. _(Source: main)_
> 🆔 `a9fb6f` [3]: Orchestrates project analysis by initializing CrawlerAgent, fetching and processing the project map, synthesizing system summaries, rendering reports, and cleaning up for five turns. _(Source: Import agent_core.py)_
</details>
...
**Role:** The module `agent_core.py` The Agent Core module orchestrates the analysis of a specified project by running a crawler to fetch the project map and processing order, synthesizing system summaries, rendering reports, and cleaning up memory states for five turns.
...
### 🧩 Interface & Logic
- **`🔌 class CrawlerAgent`**: Initializes an instance of CrawlerAgent by setting up ChromaMemory, printing initialization details, running the crawler to fetch project map and processing order, synthesizing system summary, rendering report, cleaning memories for five turns. [2]
- **`🔌 🔌 CrawlerAgent.run`**: Runs the CrawlerAgent by initializing, fetching project map and processing order, synthesizing system summary, rendering report, cleaning memories for five turns, and returning analysis completion response. [3]

### 🔗 Uses (Upstream)
- **`llm_util.py`**: Formats and sends requests to Ollama chat API, retrieves response content, strips whitespace, and returns formatted response. [4]
- **`agent_config.py`**: Defines global constants for model selection and context handling to configure agent behavior. [5]
- **`semantic_gatekeeper.py`**: Validates and filters inputs to ensure compliance before passing them for further processing, ensuring data integrity and security in downstream components. [6]
...
- **`memory_core.py`**: Enables the agent to persistently store and retrieve contextual memories for informed decision-making across turns, enhancing user interaction consistency. [8]
- **`summary_models.py`**: Organizes and manages technical documentation related to module roles, dependencies, dependents, public API entries, alerts, and claims within a software project. [9]
- **`map_synthesizer.py`**: Orchestrates synthesis of system architecture by grouping modules according to their archetypes, generating detailed group syntheses, and producing an overarching system architecture narrative through structured JSON output. [10]
- **`agent_util.py`**: Analyzes module dependencies and generates/update the project context map to summarize relationships between modules. [11]
...
> 🆔 `58ba8c` [1]: The Agent Core module orchestrates the analysis of a specified project by running a crawler to fetch the project map and processing order, synthesizing system summaries, rendering reports, and cleaning up memory states for five turns. _(Source: Synthesis (based on [2], [3]))_
> 🆔 `51f429` [2]: Initializes an instance of CrawlerAgent by setting up ChromaMemory, printing initialization details, running the crawler to fetch project map and processing order, synthesizing system summary, rendering report, cleaning memories for five turns. _(Source: class CrawlerAgent)_
> 🆔 `e0b6e5` [3]: Runs the CrawlerAgent by initializing, fetching project map and processing order, synthesizing system summary, rendering report, cleaning memories for five turns, and returning analysis completion response. _(Source: 🔌 CrawlerAgent.run)_
> 🆔 `086468` [4]: Formats and sends requests to Ollama chat API, retrieves response content, strips whitespace, and returns formatted response. _(Source: Import llm_util.py)_
> 🆔 `6075cc` [5]: Defines global constants for model selection and context handling to configure agent behavior. _(Source: Import agent_config.py)_
> 🆔 `2fd316` [6]: Validates and filters inputs to ensure compliance before passing them for further processing, ensuring data integrity and security in downstream components. _(Source: Import semantic_gatekeeper.py)_
...
> 🆔 `54f364` [8]: Enables the agent to persistently store and retrieve contextual memories for informed decision-making across turns, enhancing user interaction consistency. _(Source: Import memory_core.py)_
> 🆔 `4055db` [9]: Organizes and manages technical documentation related to module roles, dependencies, dependents, public API entries, alerts, and claims within a software project. _(Source: Import summary_models.py)_
> 🆔 `a4a722` [10]: Orchestrates synthesis of system architecture by grouping modules according to their archetypes, generating detailed group syntheses, and producing an overarching system architecture narrative through structured JSON output. _(Source: Import map_synthesizer.py)_
> 🆔 `847242` [11]: Analyzes module dependencies and generates/update the project context map to summarize relationships between modules. _(Source: Import agent_util.py)_
</details>

---
## 📦 Module: `agent_util.py`
**Role:** The module `agent_util.py` Analyzes module dependencies to generate and update the project context map.
...
- **`🔌 class ProjectSummarizer`**: Initializes the summarizer by storing the provided graph, setting the maximum number of cycles, creating an empty dictionary for contexts, and computing the processing order. [3]
- **`🔌 project_pulse`**: Creates a project graph from the target file path, summarizes it to generate module contexts and processing order [4]
- **`🔌 🔌 ProjectSummarizer.generate_contexts`**: Iterates through cycles, creating module contexts for each path in the processing order, updating them based on dependencies and critique instructions, and returning the updated contexts and processing order. [5]
- **`🔒 _create_module_context`**: Creates or retrieves module context, populates file path if missing, and returns it after contextualizing the module using ModuleContextualizer. [6]

### 🔗 Uses (Upstream)
- **`semantic_gatekeeper.py`**: Validates and filters inputs to ensure compliance before passing them for further processing in agent_core.py, agent_util.py, component_analyst.py, dependency_analyst.py, map_critic.py, map_synthesizer.py, module_contextualizer.py, task_executor.py. [7]
- **`graph_analyzer.py`**: Analyzes Python modules to construct and examine a dependency graph, facilitating the identification of code structure elements including functions, classes, imports, cross-module dependencies, and TODO comments. [8]
- **`report_renderer.py`**: Orchestrates generation of markdown report detailing project context map, including system summary and categorized modules with dependencies for presentation. [9]
- **`map_critic.py`**: Orchestrates documentation quality analysis by parsing project map content into modules and generating detailed critiques for up to three modules. [10]
- **`summary_models.py`**: Organizes and manages technical documentation related to module roles, dependencies, dependents, public API entries, alerts, and claims within the software project. [11]
- **`module_contextualizer.py`**: Analyzes module context to build usage map and set archetype based on classifier classification of the module data, informing downstream processing in agent_util.py. [12]
...
> 🆔 `168949` [1]: Analyzes module dependencies to generate and update the project context map. _(Source: Synthesis (based on [2], [4], [3], [6], [5]))_
> 🆔 `4ba765` [2]: Assigns a dictionary to the variable `ProjectGraph`. _(Source: ProjectGraph)_
> 🆔 `6d2f83` [3]: Initializes the summarizer by storing the provided graph, setting the maximum number of cycles, creating an empty dictionary for contexts, and computing the processing order. _(Source: class ProjectSummarizer)_
> 🆔 `4f36ed` [4]: Creates a project graph from the target file path, summarizes it to generate module contexts and processing order _(Source: project_pulse)_
> 🆔 `f6065e` [5]: Iterates through cycles, creating module contexts for each path in the processing order, updating them based on dependencies and critique instructions, and returning the updated contexts and processing order. _(Source: 🔌 ProjectSummarizer.generate_contexts)_
> 🆔 `7a7f22` [6]: Creates or retrieves module context, populates file path if missing, and returns it after contextualizing the module using ModuleContextualizer. _(Source: _create_module_context)_
> 🆔 `89eab1` [7]: Validates and filters inputs to ensure compliance before passing them for further processing in agent_core.py, agent_util.py, component_analyst.py, dependency_analyst.py, map_critic.py, map_synthesizer.py, module_contextualizer.py, task_executor.py. _(Source: Import semantic_gatekeeper.py)_
> 🆔 `f033eb` [8]: Analyzes Python modules to construct and examine a dependency graph, facilitating the identification of code structure elements including functions, classes, imports, cross-module dependencies, and TODO comments. _(Source: Import graph_analyzer.py)_
> 🆔 `06009f` [9]: Orchestrates generation of markdown report detailing project context map, including system summary and categorized modules with dependencies for presentation. _(Source: Import report_renderer.py)_
> 🆔 `83e231` [10]: Orchestrates documentation quality analysis by parsing project map content into modules and generating detailed critiques for up to three modules. _(Source: Import map_critic.py)_
> 🆔 `dde9ee` [11]: Organizes and manages technical documentation related to module roles, dependencies, dependents, public API entries, alerts, and claims within the software project. _(Source: Import summary_models.py)_
> 🆔 `307dcc` [12]: Analyzes module context to build usage map and set archetype based on classifier classification of the module data, informing downstream processing in agent_util.py. _(Source: Import module_contextualizer.py)_
</details>
